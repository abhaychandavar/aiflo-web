...
const OutputNode = (props: ExtendedNodeProps) => {
    ...
    const debouncedSetData = useMemo(
        () => debounce((newData) => setDebouncedData(newData), 60), // 300ms delay
        []
    );
    ...
    // Single state update function to batch all changes
    ...
    // Effect with proper dependency handling
    useEffect(() => {
    ...
    // Memoize the text output to prevent unnecessary recalculations
